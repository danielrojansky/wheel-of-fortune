...
import { getAdminEvent, deleteEvent as apiDeleteEvent, updateEvent as apiUpdateEvent } from '../../lib/api';
import { getSavedEvents, removeEvent, updateEventName } from '../../lib/eventsStorage';
...
export default function EventManager() {
  ...
  const loadEvents = async () => {
    ...
    const enriched = await Promise.all(
      saved.map(async (ev) => {
        try {
          const data = await getAdminEvent(ev.adminToken);
          const childrenList = Object.entries(data.children || {});
          const completedCount = data.assignments?.length || 0;
          return {
            ...ev,
  ...
  const handleDelete = async (adminToken) => {
    ...
    try {
      ...
      removeEvent(adminToken);
      setEvents((prev) => prev.filter((e) => e.adminToken !== adminToken));
    } catch (err) {
      alert(err.message);
    }
  };

  const handleRemoveInvalid = (adminToken) => {
    removeEvent(adminToken);
    setEvents((prev) => prev.filter((e) => e.adminToken !== adminToken));
  };

  const startEdit = (ev) => {
    setEditingId(ev.adminToken);
  ...
  const saveEdit = async (adminToken) => {
    const trimmed = editName.trim();
    if (!trimmed) return;
    try {
      await apiUpdateEvent(adminToken, trimmed);
      updateEventName(adminToken, trimmed);
      setEvents((prev) =>
        prev.map((e) => (e.adminToken === adminToken ? { ...e, eventName: trimmed } : e))
      );
      setEditingId(null);
    } catch (err) {
      alert(err.message);
    ...
    <div className="max-w-lg mx-auto px-4 py-8 space-y-6">
      ...
      {events.length === 0 ? (
        ...
        <div className="space-y-3">
          {events.map((ev) => (
            <div key={ev.adminToken} className="bg-white rounded-2xl shadow-lg p-4">
              {!ev.valid ? (
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium text-gray-400">{ev.eventName || 'אירוע לא זמין'}</p>
                    <p className="text-xs text-red-400 mt-1">האירוע נמחק או שאינו זמין</p>
                  </div>
                  <button
                    onClick={() => handleRemoveInvalid(ev.adminToken)}
                    ...
                    {editingId === ev.adminToken ? (
                      <div className="flex items-center gap-2 flex-1 ml-2">
                        <input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') saveEdit(ev.adminToken);
                        ...
                        />
                        <button
                          onClick={() => saveEdit(ev.adminToken)}
